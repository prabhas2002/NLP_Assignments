# -*- coding: utf-8 -*-
"""Copy of Assignment #2 for DS 207 (Intro to NLP): Language Modelling

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1E1BpZTG1W4AGUiKuK61YQKpgmk31Z5P2
...
def get_unigram_counts(corpus):
    ...
    unigram_counts={char1 : 0 for char1 in all_chars}
    for name in corpus:
      for char in name:
...
def get_bigram_counts(corpus):
    ...
    #init to all zero
    bigram_counts = {char1: {char2: 0 for char2 in all_chars} for char1 in all_chars}
...
def get_trigram_counts(corpus):
    ...
    #init to all zeros
    trigram_counts = {char1: {char2: {char3: 0 for char3 in all_chars} for char2 in all_chars} for char1 in all_chars}
    ...
    for name in corpus:
        ...
        for char in range(2, len(name)):
            curr_char = name[char]
            prev_char1 = name[char - 1]
            prev_char2 = name[char - 2]
            trigram_counts[prev_char2][prev_char1][curr_char] += 1
...
class UnigramModel(NGramLanguageModel):
    ...
    def __init__(self, train_text):
        ...
        self.dicts={}
        self.probs={}
        self.dicts=get_unigram_counts(train_text)
        self.probs=self.get_next_char_probabilities()
         # END CODE
    ...
    def get_next_char_probabilities(self):
        ...
        total_count = sum(self.dicts.values())
        next_char_probabilities = {char: count / total_count for char, count in self.dicts.items()}
    ...
    def get_name_log_probability(self, name):
        ...
        for i in name:
          if i in self.probs:
              if self.probs[i]!=0:
                   name_log_probability+=math.log(self.probs[i])
    ...
    def get_perplexity(self, text):
        ...
        for names in text:
          perplexity+=self.get_name_log_probability(names)

        perplexity=perplexity/tokens_length

        perplexity=math.exp(-1*perplexity)
    ...
    def generate_names(self, k, n=MAX_NAME_LENGTH, prefix=None):
        ...
        while(i<k):
            ...
            while len(name) < n and '</s>' not in name:
                next_char = self.sample_next_char(name)
                if next_char == '</s>':
                    break
                if next_char != '<s>':
    ...
    def get_most_likely_chars(self, sequence, k):
        ...
        # ADD YOUR CODE HERE
         ...
         # BEGIN CODE
        most_likely_chars=[]
        sorted_probabilities = sorted(self.dicts.items(), key=lambda x: x[1], reverse=True)
        for i in range(min(k, len(sorted_probabilities))):
            if sorted_probabilities[i][0] != '<s>':
                 most_likely_chars.append(sorted_probabilities[i][0])
   ...
   # BEGIN CODE
    def sample_next_char(self, prefix):
      rand_val = random.random()
      cumulative_prob = 0

      for char, prob in self.probs.items():
...
class SmoothedUnigramModel(UnigramModel):
    ...
    def get_next_char_probabilities(self):
        """
        Return a dictionary of smoothed probabilities for each character in the vocabulary
        ...
        """
        total_count = sum(self.dicts.values())
        vocabulary_size = len(self.dicts)

        # Compute smoothed probabilities using Add-1 smoothing
        smoothed_probs = {char: (count + 1) / (total_count + 1*vocabulary_size) for char, count in self.dicts.items()}
...
class BigramModel(NGramLanguageModel):
    def __init__(self, train_text):
        ...
        # ADD YOUR CODE HERE
        # BEGIN CODE
        self.dicts=get_bigram_counts(train_text)
        self.probs = self.get_next_char_probabilities()
        # END CODE
    ...
    def get_next_char_probabilities(self):
        ...
        for key,row in self.dicts.items():

          total_count = sum(row.values())
          if total_count!=0:
              temp={char: count / total_count for char, count in row.items()}
          else:
              temp={char: 1/len(self.dicts) for char, count in row.items()} #assigning equal probs for total count=0 (ex : key ='</s>')
          next_char_probabilities[key]=temp
    ...
    def get_name_log_probability(self, name):
        ...
        for i in range(1, len(name)):
            prev_char = name[i - 1]
            curr_char = name[i]
            if prev_char in self.probs.keys() and curr_char in self.probs[prev_char].keys() and self.probs[prev_char][curr_char] != 0:
                name_log_probability += math.log(self.probs[prev_char][curr_char])
    ...
    def get_perplexity(self, text):
        ...
        perplexity=perplexity/token_length
        perplexity=math.exp(-1*perplexity)
        return perplexity
    ...
    def get_most_likely_chars(self, sequence, k):
        ...
        # BEGIN CODE
        most_likely_chars = []
        probabilities = self.probs.get(sequence[-1], {})
   ...
   # BEGIN CODE
    def sample_next_char(self, prefix):
      """
      Sample the next character based on the model's probabilities given a prefix.

      Args:
          prefix (list): List of tokens representing the prefix.

      Returns:
          The sampled character (str).
      """

      rand_val = random.random()
      cumulative_prob = 0
      last_token=prefix[-1]
      probabilities = self.probs.get(last_token, {})
...
class LaplaceSmoothedBigramModel(BigramModel):
    ...
    def get_next_char_probabilities(self):
        ...
        for key,row in self.dicts.items():
          total_count = sum(row.values())
          if total_count!=0:
              temp={char: ((count+self.k) / (total_count + self.k * vocabulary_size))  for char, count in row.items()}
          else:
              temp={char: 1/len(self.dicts) for char, count in row.items()} #assigning equal probs for total count=0
          self.probs[key]=temp
...
class InterpolationSmoothedBigramModel(BigramModel):
    ...
    def get_next_char_probabilities(self):
        ...
        using interpolation smoothing.

        Returns:
            P: dictionary; probability distribution over characters
        """
        #interpolation between bigram and smoothed unigram.
        unigram={}
        uni_probs={}
        k=0.8  #for unigram smoothing
        vocabulary_size = len(self.dicts)
        unigram=get_unigram_counts(train_text)
        total_count = sum(unigram.values())
        smoothed_uni_probs = {char: ((count+k) / (total_count+k*vocabulary_size)) for char, count in unigram.items()}
        ...
        for key,row in self.dicts.items():
          ...
          if total_count !=0:
              #using interpolation
              temp={char: (self.lambda_1*(count/ total_count))+(self.lambda_2*smoothed_uni_probs[char]) for char, count in row.items()}
          else:
              temp={char: (self.lambda_1*1/len(self.dicts))+(self.lambda_2*smoothed_uni_probs[char]) for char, count in row.items()} #assigning equal probs for total count=0
          self.probs[key]=temp
...
class TrigramModel(NGramLanguageModel):
    def __init__(self, train_text,lambdas):
        ...
        """
         ...
         #BEGIN CODE
        self.lambda_1, self.lambda_2 , self.lambda_3= lambdas
        self.unigram = get_unigram_counts(train_text)
        self.bigram = get_bigram_counts(train_text)
        self.trigram = get_trigram_counts(train_text)
    ...
    def get_next_char_probabilities(self):
        ...
        # ADD YOUR CODE HERE
        #BEGIN CODE
        uni_next_char_probabilities={}
        bi_next_char_probabilities={}
        tri_next_char_probabilities={}

        #computing unigrams
        total_count = sum(self.unigram.values())
        uni_next_char_probabilities = {char: count / total_count for char, count in self.unigram.items()}



        #computing bigrams
        for key,row in self.bigram.items():
          total_count = sum(row.values())
          if total_count !=0:
              temp={char: count / total_count for char, count in row.items()}
          else:
              temp={char: 1/len(self.bigram) for char, count in row.items()} #assigning equal probs for total count=0
          bi_next_char_probabilities[key]=temp



       #computing interpolation
        for key,row in self.trigram.items():
            temp_next_char_probabilities={}
            for i_key,i_row in row.items():
              total_count = sum(i_row.values())
              if total_count !=0:
                  temp={char:  (self.lambda_1*(count / total_count)+self.lambda_2*bi_next_char_probabilities[i_key][char]+self.lambda_3*uni_next_char_probabilities[char]) for char, count in i_row.items()}
              else:
                  temp={char: (self.lambda_1*(1/len(self.trigram))+self.lambda_2*bi_next_char_probabilities[i_key][char]+self.lambda_3*uni_next_char_probabilities[char]) for char, count in i_row.items()}
              temp_next_char_probabilities[i_key]=temp

            tri_next_char_probabilities[key]=temp_next_char_probabilities

        return tri_next_char_probabilities
    ...
    def get_name_log_probability(self, name):
        ...
        for i in range(2, len(name)):
            most_prev ,prev_char, curr_char = name[i-2] ,name[i-1], name[i]
            if most_prev in self.probs.keys() and prev_char in self.probs[most_prev].keys() and curr_char in self.probs[most_prev][prev_char].keys() and self.probs[most_prev][prev_char][curr_char] != 0:
                name_log_probability += math.log(self.probs[most_prev][prev_char][curr_char])
    ...
    def get_most_likely_chars(self, sequence, k):
        ...
        probabilities = self.probs.get(sequence[-2], {})
        probabilities1 = probabilities.get(sequence[-1],{})
        sorted_probabilities = sorted(probabilities1.items(), key=lambda x: x[1], reverse=True)
        for i in range(min(k, len(sorted_probabilities))):
          if sorted_probabilities[i][0] != '<s>':
            most_likely_chars.append(sorted_probabilities[i][0])
...
#BEGIN CODE
    def sample_next_char(self, prefix):
      ...
      cumulative_prob = 0
      last_token=prefix[-1]
      most_last_token=prefix[-2]
      probabilities = self.probs.get(most_last_token, {})
      probabilities1= probabilities.get(last_token, {})
      if sum(probabilities1.values())==0:
            return '</s>'
      for char, prob in probabilities1.items():
          cumulative_prob += prob
          if rand_val <= cumulative_prob:
              return char
...
trigram_model = TrigramModel(train_text,lambdas=TRIGRAM_LAMBDAS)
...
class FNN_LM(nn.Module):
    ...
    def forward(self, chars):
        ...
        # YOUR CODE HERE
         ...
         #BEGIN CODE
        emb=self.C(chars)
        emb=emb.view(-1,self.emb_size*(self.ngram-1))
        temp1=self.U(torch.tanh(self.H(emb)))
        temp2=self.W(emb)
        logits=temp1+temp2
...
class NeuralNGramTrainer:
    ...
    def train_step(self):
        ...
        with torch.set_grad_enabled(True):
            for batch, (X, y) in enumerate(self.train_dataloader):
                # Send data to target device(gpu)
    ...
    def val_step(self):
        ...
        with torch.set_grad_enabled(False):
            for batch, (X, y) in enumerate(self.valid_dataloader):
                ...
                #X, y = X.to(self.device), y.to(self.device)
                #forward pass
                test_pred_logits = self.model(X.long())
                #calculate and sum loss
                loss = self.criterion(test_pred_logits, y)
                test_loss += loss.item()
        #normalized loss
        test_loss = test_loss / len(self.valid_dataloader)
    ...
    def train(self):
        ...
        for epoch in (range(self.epochs)):
          ...
          self.loss['val'].append(test_loss)
    ...
    def get_next_char_probabilities(self):
        ...
        input = torch.tensor([self.vocab[token] for token in prefix], dtype=torch.long).unsqueeze(0) #adding extra dim at 0
        logits = self.model(input)
        probs = F.softmax(logits, dim=1)
        for ind,val in enumerate(probs.squeeze(0)): #remove dim at 0
        ...
        with torch.set_grad_enabled(False):
            for batch, (X, y) in enumerate(eval_dataloader):
                ...
                for i in range(len(y_list)):
                  perplexity+=math.log(probs[i,y_list[i]].item())

        perplexity=perplexity*(-1/(tokens))
    ...
    def get_most_likely_chars(self, sequence, k):
        ...
        input = torch.tensor([self.vocab[token] for token in seq], dtype=torch.long).unsqueeze(0)
        logits = self.model(input)
        probs = F.softmax(logits, dim=1)
...
MAX_NAME_LENGTH = 13 # maximum length of name for generation
...
HID_SIZE = 300 # The size of the hidden layer
EPOCHS = 10
BATCH_SIZE = 64
...
class RNN_LM(nn.Module):
    ...
    def __init__(self,input_size, embed, hidden_size, output_size, num_layers=1, dropout=0.0):
        super().__init__()
       # ADD YOUR CODE HERE
         #BEGIN CODE
        ...
        #init layers
        self.embedding = nn.Embedding(input_size, embed)
        self.rnn = nn.RNN(embed, hidden_size, num_layers, batch_first=True, dropout=dropout)
...
class RNNTrainer:
    ...
    def val_step(self):
        ...
        with torch.no_grad():
            for batch_idx, (X, y) in enumerate(self.valid_dataloader):
                if self.use_cuda:
                    X, y = X.cuda(), y.cuda()
    ...
    def get_next_char_probabilities(self):
        ...
        input_tensor = torch.tensor([self.vocab[token] for token in start_sequence], dtype=torch.long).unsqueeze(0)
        logits = self.model(input_tensor)
        probs = F.softmax(logits.squeeze(0), dim=1)

        for idx, prob in enumerate(probs.squeeze(0)):
            char = self.vocab.get_itos()[idx]
            next_char_probabilities[char] = prob.item()
    ...
    def generate_names(self, k, n, prefix=None):
      ...
      with torch.no_grad():  # Disable gradient computation for inference
          ...
          while(i<k):
              name = []
              if prefix is None:
                  name = ['<s>'] #adding start token
              else:
                  name = prefix[:]
              while len(name) < n and '</s>' not in name:
                  name_ind = [self.vocab[token] for token in name]
                  input_tensor = torch.tensor(name_ind, dtype=torch.long).unsqueeze(0)
                  logits = self.model(input_tensor)
                  probs = F.softmax(logits.squeeze(0), dim=1)
                  next_char_index = torch.multinomial(probs[-1], num_samples=1).item()
                  ...
                  if next_char != '<s>':
                      name.append(next_char)
                      name_ind.append(next_char_index)
              name = ''.join(name)
              if name not in names:
                names.append(name)
                i+=1
  ...
  #END CODE
    ...
    def get_perplexity(self, text):
        ...
        # don't forget self.model.eval()
         ...
         #BEGIN CODE
        batch_size=32
        dataset = TextDataSet(text, self.vocab)
        ...
        # perplexty is equal to exp of total cross entropy loss
    ...
    def get_most_likely_chars(self, sequence, k):
        ...
        sequence_indices = [self.vocab[token] for token in sequence]
        input_tensor = torch.tensor(sequence_indices, dtype=torch.long).unsqueeze(0)
        logits = self.model(input_tensor)
        probs = F.softmax(logits.squeeze(0), dim=1)
        val , top_indices = torch.topk(probs[-1], k=k, dim=-1)
        most_likely_chars = [self.vocab.get_itos()[index] for index in top_indices.squeeze()]
...
class TextDataSet(Dataset):
    ...
    def __getitem__(self, idx):
        x = [self.vocab[token] for token in self.data[idx][:-1]]
        y = [self.vocab[token] for token in self.data[idx][1:]]   # target values shifted by one
        return torch.tensor(x), torch.tensor(y)

def collate_fn(batch):
    max_len = max(len(x) for x, _ in batch)
    #padding end tokens to make all data equal size in one batch
    x_pad = [torch.cat((x, torch.tensor([vocab['</s>']] * (max_len - len(x))))) for x, _ in batch]
    y_pad = [torch.cat((y, torch.tensor([vocab['</s>']] * (max_len - len(y))))) for _, y in batch]
...
train_dataset = TextDataSet(train_text, vocab)
valid_dataset = TextDataSet(validation_text, vocab)
...
dropout=0.2
model = RNN_LM(input_size,embed, hidden_size, output_size, num_layers, dropout)
