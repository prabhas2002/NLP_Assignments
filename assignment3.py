# -*- coding: utf-8 -*-
"""ONTERU_PRABHAS_REDDY_22661_assignment3.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1C0gy24FlY93RUfLon_ULxwjlYKU3y_p4
...
STUDENT_SAP_NAME  = "ONTERU PRABHAS REDDY"
STUDENT_SR_NUMBER = "22661"
...
class Tokenizer:
    ...
    def train(self, data, vocab_size):
        ...
        # ADD YOUR CODE HERE
        k=4  # 0,1,2,3 tokens are special ones
        for name in data:
          for j in name:
            if j in self.vocab:
               pass
        ...
        merges_need= vocab_size-len(self.vocab)+4
        temp=len(self.vocab)
        data= ''.join(data)
        data=list(data)
        for i in range(merges_need):
            counts=self.give_pair_counts(data)
            pair = max(counts, key=counts.get)   #getting maximum count pair
            index=temp+i
            data =self.do_merge(data,pair,index)
            self.vocab[pair[0]+pair[1]]=index


    def give_pair_counts(self,tokens):
        counts={}
        for pair in zip(tokens,tokens[1:]):
           counts[pair] = counts.get(pair, 0) + 1
        return counts

    def do_merge(self,tokens,pair,index):
        tok=[]
        i=0
        while(i<(len(tokens))):
            if i < len(tokens) - 1 and tokens[i]==pair[0] and tokens[i+1]==pair[1]:
                tok.append(pair[0]+pair[1])
                i+=2
            else:
                tok.append(tokens[i])
                i+=1
    ...
    def pad(self, tokens, length):
        ...
        while(len(tokens)<length):
          tokens.append(self.vocab[self.end])
    ...
    def unpad(self, tokens):
        ...
        # ADD YOUR CODE HERE
        new_li = [elem for elem in tokens if elem != self.end]
        return new_li
    ...
    def encode(self, string, add_start=True, add_end=True):
        ...
        while(i<len(string)):
                temp+=string[i]
                if temp in self.vocab.keys():
                    i+=1
                else:
                    temp=temp[:-1]
                    try:
                        tokens.append(self.vocab[temp])
                    except:
        ...
        if temp=='':
            try:
              tokens.append(self.vocab[string[-1]])
            except:
               tokens.append(self.vocab['<unk>'])
        else:
            try:
...
class Trainer:
    ...
    def eval_step(self, validation_dataloader):
        ...
        test_loss = test_loss / (len(validation_dataloader)*validation_dataloader.batch_size)
        return test_loss
        # END CODE
...
class RNNEncoderDecoderLM(torch.nn.Module):
    ...
    def __init__(self, src_vocab_size, tgt_vocab_size, embd_dims, hidden_size, num_layers=1, dropout=0.1):
        ...
        # ADD YOUR CODE HERE
        self.src_embedding = nn.Embedding(src_vocab_size, embd_dims)     #embedding layer for source language
        self.enc_rnn = nn.GRU(embd_dims, hidden_size, num_layers, batch_first=True)  #encoder rnn
        self.dec_rnn = nn.GRU(embd_dims, hidden_size, num_layers, batch_first=True)  #decoder rnn
        self.tar_embedding = nn.Embedding(tgt_vocab_size, embd_dims)    #embedding layer for target language
    ...
    def forward(self, inputs, decoder_inputs, decoder_hidden_state=None):
        ...
        if decoder_hidden_state == None:
           ...
           _ , enc_hidden = self.enc_rnn(src_embedded)
           decoder_hidden_state = enc_hidden

        tgt_embedded = self.dropout_layer(self.tar_embedding(decoder_inputs))
        dec_output, dec_hidden = self.dec_rnn(tgt_embedded, decoder_hidden_state)
        output_logits = (self.output_layer(dec_output))
        output_log_probs = F.log_softmax(output_logits, dim=-1) #computing log probabilities
        return output_log_probs, dec_hidden
    ...
    def log_probability(self, seq_x, seq_y):
        ...
        probs=0.0
        if seq_y[0] != 0:
            seq_y = torch.cat((torch.tensor([0]), seq_y)) #adding start token if not exist
        seq_x = seq_x.unsqueeze(0)
        seq_y = seq_y.unsqueeze(0)
        decoder_hidden_state = None
        for i in range(seq_y.shape[1]-1):
            y_ = seq_y[:,i:i+1]
            output, decoder_hidden_state = self.forward(seq_x, y_, decoder_hidden_state)
            output = output.squeeze(1)
            probs+=output.squeeze(0)[seq_y[:,i+1:i+2].squeeze(1)].item()
        return probs
        # END CODE
...
class RNNEncoderDecoderTrainer(Trainer):
    ...
    def train_step(self, x_batch, y_batch):
        ...
        if isinstance(x_batch, torch.nn.utils.rnn.PackedSequence):
             x_batch, _ = torch.nn.utils.rnn.pad_packed_sequence(x_batch, batch_first=True)
        if isinstance(y_batch, torch.nn.utils.rnn.PackedSequence):
             y_batch, _ = torch.nn.utils.rnn.pad_packed_sequence(y_batch, batch_first=True)
        ...
        for i in range(y_batch.shape[1]-1):
            ...
            output = output.squeeze(1)
            loss += self.criterion(output, y_batch[:,i+1:i+2].squeeze(1))
        ...
        with torch.set_grad_enabled(False):
            for batch, (X, y) in enumerate(validation_dataloader):
                ...
                for i in range(y.shape[1]-1):
                    y_ = y[:,i:i+1]
                    output, decoder_hidden_state = self.model(X, y_, decoder_hidden_state)
                    output = output.squeeze(1)
                    test_loss += self.criterion(output, y[:,i+1:i+2].squeeze(1))

        return (test_loss.item()/(len(validation_dataloader)*y.shape[1]))
        # END CODE
...
def rnn_greedy_generate(model, seq_x, src_tokenizer, tgt_tokenizer, max_length):
    ...
    tgt_tokens = torch.tensor([tgt_tokenizer.vocab['<st>']]).to(device)
    decoder_hidden_state = None
    while True:
        with torch.no_grad():
            output, decoder_hidden_state = model(src_tokens, tgt_tokens.unsqueeze(0), decoder_hidden_state)
            tgt_tokens = output.argmax(dim=-1)[:,-1]
            tgt_list.append(tgt_tokens.item())
            if tgt_tokens[0].item() == tgt_tokenizer.vocab['[EOS]'] or len(tgt_list) >= max_length:
                break

    tgt_list.insert(0,tgt_tokenizer.vocab['<st>']) #appending start token


    return tgt_tokenizer.decode(tgt_list)
    # END CODE
...
class AttentionModule(torch.nn.Module):
    ...
    def __init__(self, input_size):
        ...
        self.L1 = nn.Linear(self.input_size, self.input_size)   #linear layer for encoder outputs
        self.L2 = nn.Linear(self.input_size, self.input_size)   #linear layer for decoder hidden state
        self.L3 = nn.Linear(self.input_size, 1)                 #linear layer for attention scores
    ...
    def forward(self, encoder_outputs, decoder_hidden_state):
        ...
        scores = self.L3(torch.tanh(self.L2(decoder_hidden_state) + self.L1(encoder_outputs)))
        scores = scores.squeeze(2).unsqueeze(1)  #exchanging 1,2 dimensions
        weights = F.softmax(scores, dim=-1)

        return weights  #attention scores
...
class RNNEncoderDecoderLMWithAttention(torch.nn.Module):
    ...
    def attentions(self, seq_x, terminate_token, max_length):
        ...
        tgt_list= []
        scores_tensor = torch.empty(0).to('cuda')  # Create an empty tensor

        seq_x = seq_x.unsqueeze(0)

        src_embedded = self.dropout_layer(self.src_embedding(seq_x))
        keys , enc_hidden = self.enc_rnn(src_embedded)
        decoder_hidden_state = enc_hidden
        decoder_init= torch.tensor([0]).unsqueeze(0)
        decoder_init = decoder_init.to('cuda')

        while  True:
            tgt_embedded = self.dropout_layer(self.tar_embedding(decoder_init))
            decoder_hidden_state = decoder_hidden_state.permute(1,0,2)
            scores= self.attention(keys, decoder_hidden_state)
            scores_tensor = torch.cat((scores_tensor, scores), dim=0)
            context = torch.bmm(scores, keys)
            context = context.permute(1,0,2)
            dec_output, decoder_hidden_state = self.dec_rnn(tgt_embedded, context)
            output_logits = self.output_layer(dec_output)
            output_log_probs = F.log_softmax(output_logits, dim=-1)
            tgt_tokens = output_log_probs.argmax(dim=-1)[:,-1]
            tgt_list.append(tgt_tokens.item())
            if tgt_tokens[0].item() == tgt_tokenizer.vocab['[EOS]'] or len(tgt_list) >= max_length:
                break
        ...
        return tuple([scores_tensor.to('cpu'), torch.tensor(tgt_list).to('cpu')])
    ...
    def forward(self, inputs, decoder_inputs=None, decoder_hidden_state=None, output_attention=False):
        ...
        context = context.permute(1,0,2)
        decoder_hidden_state = decoder_hidden_state.permute(1,0,2)
        dec_output, dec_hidden = self.dec_rnn(tgt_embedded, context)
        output_logits = (self.output_layer(dec_output))

        output_log_probs = F.log_softmax(output_logits, dim=-1) #computing log probabilities

        if output_attention:
            return output_log_probs, dec_hidden, scores
        return output_log_probs, dec_hidden
...
def rnn_better_generate(model, seq_x, src_tokenizer, tgt_tokenizer, max_length=10, k=2, length_penalty_alpha=0.6):
    """ Given a source string, translate it to the target language using the trained model.
    ...
    while True:
        with torch.no_grad():
            output, decoder_hidden_state = model(src_tokens, tgt_tokens.unsqueeze(0), decoder_hidden_state)
            tgt_tokens = output.argmax(dim=-1)[:,-1]
            tok_k_vals , tok_k_inds = torch.topk(output[:,-1] ,k)
            tok_k_vals , tok_k_inds = list(tok_k_vals.squeeze(0)), tok_k_inds.tolist()[0]
            tok_k_inds = [str(element) for element in tok_k_inds]

            if len(tgt_dict) == 0:
                for i in range(k):
                    tgt_dict[tok_k_inds[i]] = tok_k_vals[i].item()
            else:
                prev_vals = torch.tensor(list(tgt_dict.values())).to('cuda').view(-1,1)
                prev_inds = list(tgt_dict.keys())
                curr_vals = prev_vals + torch.tensor(tok_k_vals).to('cuda').view(1,-1)
                curr_inds = []
                curr_inds = [x +' '+ y for x in prev_inds for y in tok_k_inds]
                tgt_dict = {}
                for i in range(k):
                    for j in range(k):
                        tgt_dict[curr_inds[i*k+j]] = curr_vals[i][j].item()

                top_k_values = heapq.nlargest(k, tgt_dict.values())
                tgt_dict = {key: value for key, value in tgt_dict.items() if value in top_k_values}


            if str(tgt_tokenizer.vocab['[EOS]']) in tok_k_inds or len(curr_inds[0].split()) >= max_length:
                break

    max_key, max_value = max(tgt_dict.items(), key=lambda x: x[1])
    numbers_list = [int(x) for x in max_key.split()]
    numbers_list.insert(0,tgt_tokenizer.vocab['<st>'])
    if numbers_list[-1] != tgt_tokenizer.vocab['[EOS]']:
        numbers_list.append(tgt_tokenizer.vocab['[EOS]'])

    return tgt_tokenizer.decode(numbers_list)
